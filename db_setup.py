...
db.query('UPDATE planetterp.users SET email = NULL WHERE CHARACTER_LENGTH(email) > 254')


# Add a new column will be used to keep track of users who get their email set
#   to NULL in case we decide that we want to notify those users
...
db.query("UPDATE planetterp.users SET email = NULL WHERE email = ''")

# Set user.email = None for every user with an email that appears in the db more
...
db.query('''
    ...
    SET email = NULL, email_changed = 1, send_review_email = 0
    WHERE email IN (
        SELECT email FROM (SELECT email FROM planetterp.users) AS x
        GROUP BY email
